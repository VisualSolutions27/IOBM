        ...
        public bool CreateDeviceModel(DeviceModel deviceModel)
        {
            try
            {
                using (var db = MobileManagerEntities.GetContext())
                {
                    ...
                    {
                        _eventAggregator.GetEvent<MessageEvent>().Publish(string.Format("The {0} devices model already exist.", deviceModel.ModelDescription));
                        return false;
                    }
                }
            }
        ...
        public bool UpdateDeviceModel(DeviceModel deviceModel)
        {
            try
            {
                using (var db = MobileManagerEntities.GetContext())
                {
                    ...
                    {
                        _eventAggregator.GetEvent<MessageEvent>().Publish(string.Format("The {0} device model already exist.", deviceModel.ModelDescription));
                        return false;
                    }
                    else
                    {
